...
class AI3D_OT_install_deps(jobs.JobOperator, bpy.types.Operator):
    bl_idname = "ai3d.install_deps"
    bl_label = "Instalar"
...
class AI3D_Preferences(bpy.types.AddonPreferences):
    ...
    hf_token: StringProperty(
        name="Token de Hugging Face", subtype="PASSWORD",
        description="Opcional. Solo hace falta si Hugging Face limita tus descargas")
    keep_models: BoolProperty(
        name="Mantener modelos en memoria", default=True,
        description="Generaciones siguientes mucho más rápidas; desactívalo si te falta RAM/VRAM")
    bake_device: EnumProperty(
        name="Horneado de texturas",
        items=[("CPU", "CPU", "Siempre funciona"),
               ("GPU", "GPU", "Usa la GPU configurada en Preferencias > Sistema > Cycles")],
        default="CPU")
    ...
    def draw(self, context):
        ...
        col.prop(self, "keep_models")
        col.prop(self, "bake_device")
        col.prop(self, "hf_token")
        ...
        for key, g in deps.GROUPS.items():
            ...
            op = sub.operator("ai3d.install_deps", text="Reinstalar" if ok else "Instalar",
                              icon="IMPORT")
            op.group = key
        box.label(text="Librerías: " + deps.paths["site"])
        box.label(text="Modelos: " + deps.paths["base"])
...
def draw_progress(layout, job):
    ...
    row.operator("ai3d.cancel", text="", icon="CANCEL")
...
classes = (AI3D_OT_install_deps, AI3D_Preferences)
